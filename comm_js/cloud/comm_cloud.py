...
def create_dir():
    ...
    comm_common.creat_if_dir_notexist(comm_data.writelist_dst_path)
    comm_common.creat_if_dir_notexist(comm_data.writelist_src_path)
...
def conf_offline_down_busi():
    ...
    dir_list = ((comm_data.host_rule_src_path, comm_data.comm_re_down_path),                # 规则库
                ...
                (comm_data.writelist_src_path, comm_data.writelist_dst_path),   # 白名单
                )
    while True:
        try:
...
def reg_online_down_busi():
    ...
    while True:
        ...
        # 1. 读取云库中目前都有哪些文件
        try:
            res = oss.get_bucket(bucket, prefix=uuidtmp)
        except Exception, ex:
            print ex
            comm_cloud_logger.debug("[%s]" %ex)
            continue
...
def rule_mtx_online_down_busi():
    ...
    while True:
        ...
        try:
            ...
            print "rule_mtx_online_down busi exception:"
            comm_cloud_logger.debug("rule_mtx_online_down busi exception:")
            print ex
            comm_cloud_logger.debug("[%s]" %ex)
            continue
        else:
...
def rule_vds_online_down_busi():
    ...
    while True:
        ...
        try:
            ...
            print "rule_vds_online_down busi exception:"
            comm_cloud_logger.debug("rule_vds_online_down busi exception:")
            print ex
            comm_cloud_logger.debug("[%s]" %ex)
            continue
        else:
...
def update_online_down_busi():
    ...
    while True:
        ...
        try:
            ...
            print "conf_update_online_down busi exception:"
            comm_cloud_logger.debug("conf_update_online_down busi exception:")
            print ex
            comm_cloud_logger.debug("[%s]" %ex)
            continue
        else:
...
def rule_apt_online_down_busi():
    ...
    while True:
        ...
        try:
            ...
            print "conf_apt_online_down busi exception:"
            comm_cloud_logger.debug("conf_apt_online_down busi exception:")
            print ex
            comm_cloud_logger.debug("[%s]" %ex)
            continue
        else:
...
<<<<<<< .mine
        ...
        dir_list = ((comm_data.writelist_src_path, comm_data.writelist_dst_path),)
=======
    while True:
        # 1. 读取云库中目前都有哪些文件
>>>>>>> .r5141
        try:
            ...
            print "conf_surl_online_down busi exception:"
            comm_cloud_logger.debug("conf_surl_online_down busi exception:")
            print ex
            comm_cloud_logger.debug("[%s]" %ex)
            continue
        else:
...
=======
        # 开启 规则，配置，升级包，白名单 在线下行业务
        try_catch_busi(rule_mtx_online_down_busi)
        try_catch_busi(rule_vds_online_down_busi)
        try_catch_busi(update_online_down_busi)
        try_catch_busi(rule_apt_online_down_busi)
        try_catch_busi(rule_surl_online_down_busi)
        try_catch_busi(conf_online_down_busi)
        #上述为配置文件下发业务 
        # 开启 设备状态信息 在线上行业务
        try_catch_busi(conf_online_up_busi)    
