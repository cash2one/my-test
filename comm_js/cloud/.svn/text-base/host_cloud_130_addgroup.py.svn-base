...
def cloud_down_file(oss, bucket, flist, path):
    ...
    for fname in flist:
        ...
        if res.status != 200:
            ...
            host_cloud_logger.debug("[%s] DOWN CLOUD[%s] ->FILE[%s]SUCC" %(bucket,fname,final_name))
...
def cloud_up_file(oss, bucket, local_fname, remote_fname):
    ...
    if res.status != 200:
        ...
        host_cloud_logger.debug("[%s] UP FILE[%s] -> CLOUD[%s] SUCC" %(bucket ,local_fname,remote_fname))
    ...
    for down_file in comm_common.file_filter_bysuffix([dst_path_zip],[".ok"]):
        if os.path.exists(down_file):
            try:
                ...
                host_cloud_logger.debug("==extractzip Excepiton==>file[%s] DELETE IT![%s] trace_back [%s]" %(down_file,ex,traceback.format_exc()))
                print ex
                host_cloud_logger.debug("[%s]" %ex)
                os.remove(down_file)
                continue
    ...
    while True:
        try:
            for src_dir, dst_dir in dir_list:
                #升级包 /data/gms_host/update/down/ 或 /data/gms_host/conf/down/
                if (src_dir == comm_data.host_conf_src_path or src_dir == comm_data.host_update_src_path):
                    ...
                    for src_file in specialcast_src_file_list(src_dir):
                        dst_file_list = specialcast_dst_file_list(src_file, dst_dir)  #返回阿里云路径
                        host_cloud_logger.debug("src file[%s] " % (src_file))
                        host_cloud_logger.debug("dst file list[%s]" % dst_file_list)
                        if dst_file_list == []:
                            print("**** WARNING **** multicast dst uuid list == null")
...
def cloud_flowmon_reg_down_busi():
    ...
    dst_path = comm_data.host_flowmoni_reg_src_path
    ...
    while True:
        ...
        for up_file in comm_common.file_filter_bysuffix([src_path], ['.ok']):
            ...
            try:
                ...
                shutil.copy(up_file,"/data/gms_host/mtx/mtx_error/"+backup)
                os.remove(up_file)
