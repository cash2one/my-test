...
def createzip(zfname, srcfname, passwd=zip_passwd):
...
def extractzip(zfname, dstpath, passwd=zip_passwd):
    import zipfile
...
def readzip(zfname, passwd=zip_passwd):
    data = ''
    fp = zipfile.ZipFile(zfname)
    fp.setpassword(passwd)
    for srcname in fp.namelist():
        data += fp.read(srcname)
    fp.close()
    return data
...
def is_zipfile(fname):
    try:
        ...
        # 有些 mtx 文件，文件中存在 PK 字符串。
        # 在调用 zipfile.is_zipfile 时会抛出异常。
        return False
