...
int main(int argc, char **argv)
{
	...
	/* 链接数据库 等待1.5s */
	conn = grd_connectdb("127.0.0.1", "6379", "123456");
	if (conn == NULL) {
...
#if 0
	/* 选择一个数据库编号 */
	retn = grd_selectdatabase(conn, 9);
	if (retn < 0) {
		goto EXIT;
	}
#endif
    ...
    retn = grd_range_list(conn, "T_PM_EVENTLOG_ATTACK_ATTACK", 20, show_values, &value_num);
    if (retn < 0) {
        goto EXIT;
    }
...
#if 0
	/* 查看制定key 是否存在？*/
	retn = grd_is_key_exist(conn, "foo");
	if (retn < 0) {
	...
	retn = grd_create_or_replace_hash_table(conn, "ffff", 2, hfields, hvalues);
	if (retn < 0) {
		printf("create error\n");
	}
	free(hfields);
	...
	retn = grd_hincrement_one_field(conn, "ffff", "event_type1");
	if (retn < 0) {
		//...
	}

	/* 给zset 表相应的 字段值自增1 */
	retn = grd_increment_sort_set(conn, "T_PM_EVENTLOG_ATTACK_TOP5", "1.1.1.1");
	if (retn  < 0) {
...
#if 0
	retn = grd_get_list_cnt(conn, "T_PM_EVENTLOG_AV");
	if (retn < 0) {
		goto EXIT;
	}
	printf("cnt = %d\n", retn);

	retn = grd_trim_list(conn, "T_PM_EVENTLOG_AV", 0, 1);
