...
int write_statistical_flow(void *data, void *arg)
{
	...
	/* flow_time_str */
	localtime_r(&(conn->flow_time),flow_time_p);
	...
	sprintf(g_a_line_fdata, "%s|%d|%s|%.2lf|%.2lf|%u|%u|%d|%d\n", 
...
void init_statistics_list(list_t *list)
{
	...
	node = calloc(1, sizeof(f_statistical_t));
	node->policy_type = 2;
	node->protocol_type = 0;	
	node->is_display = 1;
//	strncpy(node->redis_tname, KEY_EVENT_FLOW_ALL, KEY_NAME_SIZ-1);
	memset(node->redis_tname,0,KEY_NAME_SIZ-1);
	node->dynamic = 0;
...
int make_flow_file(flow_conn_t *conn)
{
	...
	struct tm file_time_p[1] = {0};
	int retn = 0;

	/* 制作文件名称 */
	localtime_r(&(conn->flow_time),file_time_p);
