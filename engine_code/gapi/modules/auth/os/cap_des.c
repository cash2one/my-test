...
char *des_descrypt(char *key,char *filename,char output[150])
{
	...
	if(fd < 0) {
		...
		while(1) {
			...
			/*for(i = 0; i < (int)strlen(buf); i++) {   
			  ...
			  printf("\n");*/
			CurCalc_DES_Decrypt((U08 *)key, (U08 *)buf, (U08 *)putdata);
			strcat(output, putdata); 
		}
		//	printf("%s",output);   
	//	printf("\n");
#if 0 
...
int main()
{
	char file[50]="/gms/auth/conf/tmp_des";
	char key[11]="chanct-gms";
	char getfile[150]={0};
	des_descrypt(key,file,getfile);
	printf("%s\n",getfile);
}
