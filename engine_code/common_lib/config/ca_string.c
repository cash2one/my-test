...
static inline int _ca_strncpy(char *dst, int size, const char*src, int len)
{
	int l;
...
int ca_strncpy(char *dst, int size, const char *src, int len)
{
	...
	return _ca_strncpy(dst, size, src, len);
}
...
char *ca_memrstr(const char * haystack, int range, const char * needle, int needle_len)
{
	...
	while(p >= haystack) {
		p = ca_memrchr(haystack, (*)(p + 1), *needle);
		if (p) {
			if(!memcmp(p, needle, needle_len)) {
				return p;
			} else {
...
/*
char* ca_memrchr(const char *begin, const char *end,char ch)
{
	const char *p = end - 1;
...
char  * 
ca_memrchr(const void *s, char c, int n)
{
	char *p =NULL;
	char *ps = NULL;
