...
static void Encode PROTO_LIST((unsigned char *, unsigned long *, unsigned int));
static void Decode PROTO_LIST((unsigned long *, unsigned char *, unsigned int));
...
int md5_file(const char *path, unsigned char *checksum)
{
	...
	if (NULL == path || NULL == checksum) {
		merror("md5_file: arg error\n");
		return -1;
	}

	fp = fopen(path, "rb");
	if (NULL == fp) {
		merror("open %s: %s", path, strerror(errno));
		return -1;
	}
	...
	mdump("md5 checksum of %s: ", path);
	for (i = 0; i < 16; i++)
		mdump("%02x", *(checksum + i));
	mdump("\n");
