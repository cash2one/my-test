...
static inline unsigned int DJB_hash(unsigned char *str)
{
...
static int search_name(unsigned int *so_name_array, int array_idx, char *so_name)
{
	unsigned int so_name_id = DJB_hash((unsigned char *)so_name);
...
int common_load_so(so_func_desc *sfd)
{
	...
	unsigned int so_name_array[64] = {0};
	so_func_desc *sfdp = sfd;
	int idx = 0;
	int same_idx = 0;
	char *error = NULL;
	for (idx = 0; 
		(sfdp = sfd + idx) != NULL && NAME_VALID(sfdp->func_name); 
		idx++) {
...
int common_close_so(so_func_desc *sfd)
{
	...
	while (NAME_VALID(sfdp->func_name)) {
		if (sfdp->handle != NULL 
			&& search_handle(so_handle_array, idx, sfdp->handle) < 0) {
			dlclose(sfdp->handle);
		}
		sfdp->handle = NULL;
