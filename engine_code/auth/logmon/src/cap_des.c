...
{
    FILE *tmp_sign_fp = NULL;
    int size = 0;                                                                                                     
    FILE *fp = NULL;
    char sign_buffer[2048] = {0};
    if(NULL == (fp = fopen(authfile_path, "rb"))) {
        printf("cannot open [%s] file!\n", authfile_path);
        return -1;
    } else {
        size = fseek(fp, (-atoi(sign_len)), SEEK_END);
        printf("size:%d\n", size);                                                        
        int succ_read = 0;
        if ((succ_read = fread(sign_buffer, atoi(sign_len), 1, fp)) != 1) {
            printf("**********read error!\n");
            return -1;
            //fclose(fp);
        } else {                    
            printf("succ_read = %d\n", succ_read);
            tmp_sign_fp = fopen("/gms/auth/conf/tmp_sign_file", "w+");
            fwrite(sign_buffer, atoi(sign_len), 1, tmp_sign_fp);
            fclose(tmp_sign_fp);
        }
        fclose(fp);
    }
...
{
    FILE *tmp_des_fp = NULL;
    FILE *fp = NULL;
    int size = 0;
    char szCipherBlock[1024] = {0};
    ...
    if(NULL == (fp = fopen(authfile_path, "rb"))) {
        ...
        printf("size:%d\n", size);
        fseek(fp, 0, SEEK_SET);
        if(fread(szCipherBlock, (size - atoi(sign_len)), 1, fp) != 1) {
            printf("read error!\n");
            return -1;
            //fclose(fp);
        } else {
            tmp_des_fp = fopen("/gms/auth/conf/tmp_des_file", "w+");
            fwrite(szCipherBlock, (size - atoi(sign_len)), 1, tmp_des_fp);
            fclose(tmp_des_fp);
        }
        fclose(fp);
    }
...
{
    char rsa_cmd[1024] = {0};
    char pubkey[] = {"/gms/auth/conf/rsa.1024.pub"};
    char tmp_sign_file[] = {"/gms/auth/conf/tmp_sign_file"};
    char tmp_des_file[] = {"/gms/auth/conf/tmp_des_file"};
    char tmp_file[] = {"/gms/auth/conf/tmp_file"};
    char re_sign[20] = {0};
    sprintf(rsa_cmd, "openssl dgst -verify %s -sha1 -signature %s %s > %s", pubkey, tmp_sign_file, tmp_des_file, tmp_file);
    sys_cmd(rsa_cmd);
    FILE *fp = NULL;
    if(NULL == (fp = fopen(tmp_file, "rb"))) {
        printf("cannot open [%s] file!\n", tmp_file);
        return -1;
    } else {
        fseek(fp, 0, SEEK_END);
        ...
        size = ftell(fp);
        fseek(fp, 0, SEEK_SET);
        fread(re_sign, size - 1, 1, fp);
        printf("re_sign:%s\n", re_sign);
        if((strcmp(re_sign, "Verified OK")) == 0) {
            *ret = 1;
        } else {
            *ret = 0;
        }
...
#if 0    
    char sign_name[50] = {0};                                                                                     
    char authfile_path[FILE_PATH_SIZE] = {0};                                                                    
    char sign_len[10] = {0};
    get_sign_name(sign_name);                                                                                     
    sprintf(authfile_path, "%s%s.gau", auth_conf_cfg.auth_check_path, sign_name);                                
    rintf("auth_file_path:%s\n", authfile_path);       
    get_sign_len(sign_name, sign_len);
#endif
    int l_ret = 0;
    int f_ret = 0;
    l_ret = get_last_file(authfile_path, sign_len);
    f_ret = get_former_file(authfile_path, sign_len);
    int retn = 0;
    if(l_ret == 0 && f_ret == 0) {
        rsa_verify(&retn);
        printf("retn:%d\n", retn);
        ...
        int fd;
        if(retn == 1) {
            fd = open("/gms/auth/conf/tmp_des_file", O_RDONLY);
            if(fd < 0) {
                printf("open /gms/auth/conf/tmp_des_file error!\n");
                return -1;
        } else { 
            //bzero(buftmp,1024);
            while(1) {
            ...
            printf("\n");*/
                CurCalc_DES_Decrypt((U08 *)"chanct-gms", (U08 *)buf, (U08 *)putdata);
                strcat(plainblock, putdata);    
            }
            plainblock[strlen(plainblock)]='\0';
            printf("%s",plainblock);
            printf("\n");
   #if 0 
            remove("/gms/auth/conf/tmp_sign_file");
            remove("/gms/auth/conf/tmp_des_file");
            remove("/gms/auth/conf/tmp_file");
   #endif
            }
            close(fd);
        } else {
